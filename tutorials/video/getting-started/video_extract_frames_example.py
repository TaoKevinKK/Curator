...
from nemo_curator.stages.video.clipping.clip_frame_extraction_v1 import ClipFrameExtractionV1Stage
...
def create_video_extracting_pipeline(args: argparse.Namespace) -> Pipeline:  # noqa: PLR0912, C901
    # Define pipeline
    pipeline = Pipeline(name="video_extracting", description="Extract frames from videos")
    ...
    pipeline.add_stage(
        FixedStrideExtractorStage(
            max_clip_sec=900.0,
            clip_stride_sec=900.0,
            # 小于max_clip_sec的clip会被过滤掉
            min_clip_sec=2.0,
            max_clips_per_video=1000000,
        )
    )

    purposes = []
    pipeline.add_stage(
        ClipFrameExtractionV1Stage(
...
def main(args: argparse.Namespace) -> None:
    pipeline = create_video_extracting_pipeline(args)
